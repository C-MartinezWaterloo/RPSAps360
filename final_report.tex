...
% TabTransformer baseline (full features; full train; seed=42)
...
% Transformer baseline (full features; full train; seed=42)
...
% Qualitative plot data (auto-generated; keeps the main report readable)
\newif\ifannqual
\IfFileExists{ann_qual_data.tex}{
  \annqualtrue
  \input{ann_qual_data.tex}
}{
  \annqualfalse
}
...
\begin{itemize}
  ...
  \item \textbf{TabTransformer} applies self-attention to contextualize categorical embeddings for tabular prediction (\cite{huang2020tabtransformer}).
...
\subsection{Additional baselines: FM, TabNet, TabTransformer, FT-Transformer}
\textbf{FM} models all pairwise feature interactions via low-rank factors, often strong for sparse/high-cardinality problems. \textbf{TabNet} uses sequential attentive feature selection with sparse masks. \textbf{TabTransformer} applies self-attention to categorical embeddings (contextualizing categories using other columns) and then predicts with an MLP. \textbf{FT-Transformer} tokenizes each feature into a vector and applies self-attention to learn cross-feature interactions (ours: \(d=64\), 3 layers, 4 heads, ff-mult 4, dropout 0.1, AdamW \(lr=10^{-3}\), \(wd=10^{-4}\), batch 2048, 6 epochs).

\section{Evaluation protocol and metrics}
\textbf{Train/val/test splits.} We use (i) a standard 70/15/15 random split and report robustness by repeating over multiple model seeds and split seeds, and (ii) a time-based split that sorts by (\texttt{TransactionYear}, \texttt{Quarter}) and partitions 70/15/15 by time order (with a seeded random tiebreak within the same year/quarter). All reported results use the full training split (no max-sample cap) unless explicitly stated. We tune using validation only and treat the test split as a one-time evaluation.
...
\textbf{All models.} Table~\ref{tab:all-models} summarizes performance for all tested baselines, including interaction models (FM/DeepFM), attention-based TabNet, and transformer-based TabTransformer/FT-Transformer.
...
\end{tabular}
\vspace{-2mm}
\caption{Performance summary on the full feature set (no training cap). ANN/Hedonic are averaged over robustness runs (random: 21; time: 9). DeepFM is averaged over 3 full-train seeds (0/1/42). FM, TabNet, TabTransformer, and FT-Transformer are single full-train runs (seed=42).}
...
\ifannqual
To contextualize quantitative error, we show three test examples chosen by absolute percentage error (APE) percentiles (5th/50th/95th). On the random-split test set, the ANN’s APE distribution has median \AnnApeP50\%, p90 \AnnApeP90\%, p95 \AnnApeP95\%, and p99 \AnnApeP99\%. Figure~\ref{fig:ann-qual} visualizes predictions/residuals and shows that errors are larger for the most expensive decile (rarer, more heterogeneous properties).
\else
To contextualize quantitative error, we show representative test examples (Table~\ref{tab:qual}). To regenerate the qualitative plots used in this report, run \texttt{python make\_ann\_qual\_data.py --data ann\_tensors\_full.pt}.
\fi
\begin{table}[t]
\centering
\small
...
\addplot+[only marks, mark=o, mark size=1.0pt, color=blue!70!black, opacity=0.55] table[x=true,y=pred] {\AnnPredScatter};
...
\end{tikzpicture}
\vspace{-2mm}
\caption{Predicted vs true (160 sampled test points).}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\linewidth}
\centering
...
\end{tikzpicture}
\vspace{-2mm}
\caption{Residual histogram (full test set).}
\end{subfigure}
...
\addplot+[mark=*, mark size=1.2pt, color=purple!70!black] table[x=decile,y=acc] {\AnnByPriceDecile};
\end{axis}
\end{tikzpicture}
\vspace{-2mm}
\caption{Accuracy by true-price decile (random-split test set).}
\end{subfigure}

\vspace{-2mm}
\caption{Qualitative diagnostics for the ANN (illustrative run; random split, seed=42, best epoch by validation MSE).}
\label{fig:ann-qual}
\end{figure}
\fi
...
\begin{axis}[
  ...
  width=\linewidth,
  height=3.0cm,
  xlabel={Time-split test RMSE(log)},
...
\end{tikzpicture}
\vspace{-2mm}
\caption{New-data proxy: time-split RMSE(log) across all models (Table~\ref{tab:all-models}).}
\label{fig:time-all-models}
\end{subfigure}

\vspace{-2mm}
\caption{Analysis: feature richness matters (left), the ANN relies heavily on size/time/location (middle), and time-based evaluation changes the model ranking (right).}
\label{fig:analysis}
\end{figure*}
...
\begin{itemize}
  ...
  \item \textbf{ANN vs baselines.} FM improves over hedonic regression by adding pairwise interactions. DeepFM, TabTransformer, and FT-Transformer are strong neural baselines, but the ANN remains best on random splits (Table~\ref{tab:all-models}). On the time split, FT-Transformer and TabTransformer are competitive in our single runs (\FtTimeTestRmse, \TabtrTimeTestRmse), suggesting attention-based models may help under distribution shift (though multi-seed runs would be needed for a fair robustness comparison). TabNet underperforms in our setting, highlighting sensitivity to optimization/hyperparameters for this dataset.
  \item \textbf{New-data behavior.} Time splits are harder due to distribution shift; the ANN remains meaningfully better than hedonic regression (Figure~\ref{fig:time-all-models}), suggesting the improvement is not only due to random mixing across years.
...
\newblock \emph{arXiv:2012.06678}, 2020.
